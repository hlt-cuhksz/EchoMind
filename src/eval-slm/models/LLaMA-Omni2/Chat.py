...
def generate_response(
    ...
    model_path="models/LLaMA-Omni2-7B",
    temperature=0,
    top_p=None,
    top_k=None,
    num_beams=1,
...
class SpeechDecoder:
    def __init__(self,
        model_dir='models/cosy2_decoder/',
        device="cuda",
        hop_len=None,
        load_jit=False,
        load_trt=False,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output_path", type=str, required=True, help="输出音频文件路径")
    parser.add_argument("--model_path", type=str, default="/share/workspace/EQ-SLM/EQ-Bench/src/03-eval-slm/models/LLaMA-Omni2/models/LLaMA-Omni2-7B", help="模型路径")
    parser.add_argument("--system_prompt", type=str, default="You are a helpful assistant.", help="系统提示词")
    parser.add_argument("--user_instruction", type=str, default=None, help="用户文本（可选）")
    parser.add_argument("--temperature", type=float, default=0, help="生成温度")
    parser.add_argument("--top_p", type=float, default=None, help="核心采样参数")
    parser.add_argument("--top_k", type=int, default=None, help="选择前k个最可能的token")
    ...
    parser.add_argument("--s2s", action="store_true", default=False, help="是否使用语音到语音模式")
    parser.add_argument("--model_dir", type=str, default='models/cosy2_decoder/', help="CosyVoice模型目录")
    parser.add_argument("--lang", type=str, default="en", choices=["zh", "en"], help="语言选择")
    parser.add_argument("--stream", action="store_true", help="流式生成")
    parser.add_argument("--hop_len", type=int, default=None, help="跳跃长度")
    parser.add_argument("--load_jit", action="store_true", help="加载JIT模型")
    parser.add_argument("--load_trt", action="store_true", help="加载TRT模型")
    parser.add_argument("--load_onnx", action="store_true", help="加载ONNX模型")
    ...
    try:
        ...
        print(f"生成的文本: {result['prediction']}")
        ...
        # 保存音频
        torchaudio.save(args.output_path, tts_speech, 24000)
